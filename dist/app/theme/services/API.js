...
(function () {
    ...
    angular.module('BlurAdmin.theme').factory('API', API);
    ...
    function API($http, APIURL) {
        ...
        this.doLogin = function (user, cb) {
            $http.post(this.baseURL + '/users/login', user).then(function (response) {
                cb(response);
            });
        };
        ...
        this.doRegister = function (user, cb) {
            $http.post(this.baseURL + '/users/register', user).then(function (response) {
                cb(response);
            });
        };
        ...
        this.getSafeRange = function (cb) {
            $http.get(this.baseURL + '/data/saferange').then(function (response) {
                cb(response.data);
            });
        };
        ...
        this.getData = function (obj, type, cb) {
            ...
            $http.post(this.baseURL + '/data/hour?type=' + type, obj).then(function (response) {
                // cb(response);
                // console.log(response)
                if (response.data.success) {
                    var keys = response.data.data.map(function (i) {
                        return i._id;
                    });
                    var a = [].concat(_toConsumableArray(Array(24).keys())).map(function (i) {
        ...
        this.getMonthData = function (obj, cb) {
            console.log(obj)
            $http.post(this.baseURL + '/data/daily', obj).then(function (response) {
                // cb(response);
                if (response.data.success) cb(null, response.data.data);else cb(response.data.msg);
            });
        };

        this.getTableData = function (obj, cb) {
            $http.post(this.baseURL + '/data', obj).then(function (response) {
                // cb(response);
                if (response.data.success) cb(null, response.data.data, response.data.pages);else cb(response.data.msg);
            });
        };
