...
(function () {
  ...
  angular.module('BlurAdmin.pages.dashboard', []).config(routeConfig).controller('DashboardController', dashboardController);
  ...
  function dashboardController($scope, $timeout, baConfig, layoutPaths, Utils, API, toastr, $rootScope, ngProgressFactory, SocketURL, PROGRESSBAR_COLOR) {
    ...
    $scope.generateHTML = function (n) {
      return "<b>" + n.name + "</b><br>" + "<b> Nhiệt độ: " + n.now.temp + "°C</b><br>" + "<b> Độ ẩm: " + n.now.hum + "%</b><br>" + "<b> AQI: " + n.now.pm2 + "</b><br>" + "<b> Thời gian: " + n.now.lastUpdate + "</b><br>" + "<b> Trạng thái: " + (n.connected ? 'Đã kết nối' : 'Ngắt kết nối') + "</b>";
    };
    ...
    $timeout(function () {
      ...
      $scope.submit = function () {
        ...
        if ($scope.currentNode) $scope.currentNode.status = Utils.getPollutionLevel($scope.currentNode['now']['pm2']);
      };
